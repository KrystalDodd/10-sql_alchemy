...
def tobs():
    ...
    tempobs_data = session.query(Measurement.date, Measurement.tobs).\
        filter((Measurement.station == "USC00519281"), (Measurement.date >= year_ago)).\
            order_by(Measurement.date.desc()).all()
    
    # tempobs_dict = {data[0]: data[1] for data in tempobs_data}
    tempobs_list = []
    for data in tempobs_data:
        tempobs_dict ={}
        tempobs_dict["Date"] = data.date
        tempobs_dict["Temperature"] = data.tobs
        tempobs_list.append(tobs_dict)
